# Source improved-1/svm_classification
#
# Modified to increase iterations due to previous warnings in log.
# Also added class_weight='balanced' to handle potential class imbalance issues from warnings.
# Removed .toarray() to keep the data in sparse format.
...
for project in PROJECTS:
    ...
    for REPEAT in REPEAT_TIMES:
        print(f"\n--- [Improved-{ID}] Running Linear SVM + TF-IDF for project: {project} with {REPEAT} repeats ---")
        ...
        for repeated_time in range(REPEAT):
            ...
            clf = LinearSVC(max_iter=5000, class_weight='balanced', random_state=51003)  # Increase max_iter to avoid warnings
            grid = GridSearchCV(
                clf,
                params,
            ...
            best_clf = LinearSVC(**grid.best_params_, max_iter=5000, class_weight='balanced', random_state=51003)
