...
class SVMAlgorithm(BaseAlgorithm):
    ...
    def train(self, repetitions: int = 10, seed: Optional[int] = 51003) -> dict[str, Any]:
        ...
        for repeated_time in tqdm(range(repetitions), desc=f"Training {repetitions} repetitions"):
            ...
            clf = LinearSVC(max_iter=5000, class_weight='balanced', random_state=seed)  # Increase max_iter to avoid warnings
            grid = GridSearchCV(
                clf,
                params,
                n_jobs=-1,
            ...
            self.model = LinearSVC(**grid.best_params_, max_iter=5000, class_weight='balanced', random_state=seed)
