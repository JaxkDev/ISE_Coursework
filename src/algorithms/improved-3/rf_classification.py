...
for project in PROJECTS:
    ...
    if not os.path.exists(RESULTS_DIR):
        os.makedirs(RESULTS_DIR)
    out_csv_name = f'{RESULTS_DIR}{project}_RF.csv'
    ...
    for REPEAT in REPEAT_TIMES:
        print(f"\n--- [Improved-{ID}] Running Random Forest + TF-IDF for project: {project} with {REPEAT} repeats ---")
        ...
        for repeated_time in range(REPEAT):
            ...
            # --- 4.3 Random Forest model & GridSearch ---
            clf = RandomForestClassifier(class_weight='balanced', random_state=51003) 
            grid = GridSearchCV(
                clf,
                params,
            ...
            best_clf = RandomForestClassifier(**grid.best_params_, class_weight='balanced', random_state=51003)
