...
## This file is a modified version of the lab 1 solution provided at 
## https://github.com/ideas-labo/ise-lab-solution/blob/main/lab1/br_classification.py
## Modifications were only done to make it work. (path changes and dataset toarray(), sparse was not allowed only dense.)
## Errors from original: "TypeError: Sparse data was passed for X, but dense data is required. Use '.toarray()' to convert to a dense numpy array."

## Added two loops (second inside the first one) to repeat the experiments with different random seeds for train/test split, and different numbers of repeated experiments.:
## 1) Loop over 5 projects (pytorch, tensorflow, keras, incubator-mxnet, caffe)
##   2) Loop over different numbers of repeated experiments (5, 10, 25, 50, 100)

## This does take a while to run, but is only needed once to get the results.
...
if not os.path.exists('./src/baseline/tmp'):
    os.makedirs('./src/baseline/tmp')
...
for project in PROJECTS:
    ...
    pd_tplusb.to_csv('./src/baseline/tmp/'+project+'_Title+Body.csv', index=False, columns=["id", "Number", "sentiment", "text"])
    ...
    # 1) Data file to read
    datafile = './src/baseline/tmp/' + project + '_Title+Body.csv'
    ...
    for REPEAT in REPEAT_TIMES:
        print(f"\n--- Running Naive Bayes + TF-IDF for project: {project} with {REPEAT} repeats ---")
